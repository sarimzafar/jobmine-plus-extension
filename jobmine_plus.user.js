...
/*========Table of Contents============
   ...
   _CONSTANTS_
   _PROTOTYPE_FUNCTIONS_
   _PARSE_PAGE_
   _USERSCRIPTS_BRIDGE_
   _JQUERY_SIZZLE_TABLESORTER_LIBRARIES_
   _DEBUGGING_
   _STORAGE_MANAGER_
   _JOBS_QUEUE_
   _FUNCTIONS_
...
var CONSTANTS = {
   VERSION     : "2.0.0",
   DEBUG_ON    : true,
   PAGESIMILAR : "https://jobmine.ccol.uwaterloo.ca/psc/SS/EMPLOYEE/WORK/",
};
...
var LINKS = {
   HOME        : CONSTANTS.PAGESIMILAR + "h/?tab=DEFAULT",
   LOGOUT      : CONSTANTS.PAGESIMILAR + "?cmd=logout",
   DOCUMENTS   : CONSTANTS.PAGESIMILAR + "c/UW_CO_STUDENTS.UW_CO_STUDDOCS",
   PROFILE     : CONSTANTS.PAGESIMILAR + "c/UW_CO_STUDENTS.UW_CO_STUDENT",
   SKILLS      : CONSTANTS.PAGESIMILAR + "c/UW_CO_STUDENTS.UW_CO_STUDENT?PAGE=UW_CO_STU_SKL_MTN",
   SEARCH      : CONSTANTS.PAGESIMILAR + "c/UW_CO_STUDENTS.UW_CO_JOBSRCH",
   LIST        : CONSTANTS.PAGESIMILAR + "c/UW_CO_STUDENTS.UW_CO_JOB_SLIST",
...
if (PAGEINFO.TITLE === "JobMine | University of Waterloo" && UTIL.idExists("userid") ) {
   ...
   switch(PAGEINFO.TITLE) {
      ...
      case "Student Data":          PAGEINFO.TYPE = PAGES.PROFILE;      break;
   ...
   runNextJob : function(){
      if (!this.isEmpty() && !this.isRunning) {
         ...
         if (number == null) {
            ...
            this.number = number;
            this.command = item.commandTemplate.replace("%", number);
         }
         if(item.callback) {
...
function ajaxComplete(name) {  
   ...
   if (name != "#ICSave" && (!JOBQUEUE.isRunning || item == null)) {
      return;
   }
   var jobFinished = false;
   var whitePopupShown = isPopupShown(true);
...
function initRowDeletion() {
   ...
   $("form:eq(0) > div.jbmnplsTable div.delete").live("click", function (){
      ...
      if(confirm("Are you sure you want to delete this row?\nYOU CANNOT UNDO THIS AFTER YOU AGREE!"))
      {
         ...
         tr.parent().find("div.delete").attr("disabled", "true");
...
function applyHighlight(){
   $("form:eq(0) div.jbmnplsTable td:contains('Already Applied')").parent().css("background","#a3f57f");
}
...
function JbmnplsTable (defaultName, tableID, objectToAppendTo) {
   ...
   this.cname = defaultName != null ? defaultName.replace(/\s/g, "_") : null;
 ...
 */
JbmnplsTable.prototype.parseTable = function(tableID) {
   if (tableID.empty() || !UTIL.idExists(tableID) || UTIL.getID(tableID).tagName.toUpperCase() != "TABLE") {
      return false;
   }
   ...
   //Incase some of the ids do not exist
   try{
   ...
   //Data
   this.cname = this.name.replace(/\s/g,"_");
...
JbmnplsTable.prototype.build = function() {
   ...
   html +=     "</div><div class='jbmnplsTableControls'><span class='pages'>"+(this.pageControls != null ? this.pageControls : "")+"</span><a class='options' href='#'>Customize</a> | <a class='options' href='#'>Export</a>";
   ...
   for (var h = 0; h < this.columns; h++) {
      if (h < this.headers.length) {
         ...
         if (this.hasCheckboxes && header == "{CHECKBOXES}") {
            header = "";
         }
   ...
   html +=     "<a class='options' href='#'>Customize</a> | <a class='options' href='#'>Export</a>"+controlsHTML+"</div>";
   html +=     "</div></div>";
   this.html = html;
   this.hasBuilt = true;
   html = null;
...
JbmnplsTable.prototype.appendTo = function(obj) {
   ...
   if (this.rows > 1) {
      ...
      //Append events
      this.jInstance.bind("sortEnd", this.updateCheckboxes);
      this.jInstance.find("tbody:eq(0) tr").live("click", this.onClick);
   }
   return this;
}
               ...
               table.applyFilter("", TABLEFILTERS.deleteRow)
                    ...
                    .addControlButton("Select All", function(){
                     ...
                     .addControlButton("Delete Selected", function(){
                        ...
                        setTitle("Deleting: "+progress);
                        showPopup(false, "Deleting all the short listed jobs.<br/>Progress: "+progress+"<br/><span style='color:blue;'>You can cancel by refreshing.</span>", "Please Be Patient", 500, 300);
...
var CSSOBJ = {
   ...
   "#jbmplsControlPanel a": {
      "font-size": "11px",
   },
   "#jbmplsControlPanel a:hover" : {
      "color" : "#6f6f6f",
   },
   /**
    *    Jobmine Plus Tables
   ...
   "div.jbmnplsTable table td .delete.disabled, div.jbmnplsTable table td .delete[disabled='true']" : {
