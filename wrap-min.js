 ...
 *   Redirects
 */
function focusPass(){document.getElementById("userid").removeEventListener("focus",focusPass,false);document.getElementById('pwd').focus();}if(window.location.href.indexOf("jobmine.ccol.uwaterloo.ca/servlets/iclientservlet/ES/")!=-1){window.location.href="https://jobmine.ccol.uwaterloo.ca/servlets/iclientservlet/SS/?cmd=login";return;}else if(document.getElementById("search")){var user=getCookieValue('LASTUSER');if(user!=-1&&document.getElementById("userid")){document.getElementById("userid").value=user;document.getElementById("userid").addEventListener("focus",focusPass,false);}return;}else if(window.location.href=='https://jobmine.ccol.uwaterloo.ca/servlets/iclientservlet/SS/?cmd=start&'){document.getElementsByTagName("html")[0].innerHTML="<body>"+white_overlay("<div style='text-align:justify;'>Welcome to Jobmine Plus! No Jobmine Plus is not slower than Jobmine (well only by a bit since it does extra enhancements after), it is about the same timing. If you don't like this because it is slower, then you will miss out on all the great features. By the time you read this, the next page would have loaded.</div>",21,false)+"</body>";document.getElementById('popupWhiteContainer').style.display="block";document.getElementById('popupWhiteContent').style.top="-130px";window.stop();writeCookie("LASTUSER",getCookieValue("SignOnDefault").toLowerCase());setTimeout(function(){var default_page=getCookieValue('DEFAULT_PAGE');switch(default_page){case'in':window.location.href=INTERVIEW_PAGE;break;case'js':window.location.href=JOB_SEARCH_PAGE;break;case'dc':window.location.href=DOCUMENT_PAGE;break;case'jl':window.location.href=JOB_SHORT_PAGE;break;case'rk':window.location.href=RANKING_PAGE;break;case'pr':window.location.href=PROFILE_PAGE;break;case'wr':window.location.href=WORK_REPORT_PAGE;break;default:window.location.href=APPLICATION_PAGE;break;}},1);return;}
